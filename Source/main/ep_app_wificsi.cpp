...
void EPAppWifiCsi::enable_csi(uint32_t currentMillis) {
    ...
    // -----------------------------------------------------------------------
    // Use WifiM::getWifiStaStatus() — this is set to true only after
    // IP_EVENT_STA_GOT_IP fires, which is the definitive "STA is up" signal.
    // Works correctly in APSTA mode unlike esp_wifi_sta_get_ap_info().
    if (!WifiM::getWifiStaStatus()) {
        ESP_LOGW(TAG, "enable_csi: STA not ready yet (WifiM::wifi_sta_ok=false), retry in 2s");
